...
// void cmd_vel_callback(vel_vector cmd_vel);
// vel_vector get_twist_vel(double wl_desired_speed, double wr_desired_speed);
...
// void cmd_vel_callback(vel_vector cmd_vel){
//   double vel_lin = cmd_vel.lin;
//   double vel_ang = cmd_vel.ang;
...
// vel_vector get_twist_vel(double wl_desired_speed, double wr_desired_speed) {
//   vel_vector vv;
//   vv.lin = WHEEL_RADIUS*(wr_desired_speed + wl_desired_speed) / 2;
//   vv.ang = WHEEL_RADIUS*(wr_desired_speed - wl_desired_speed) / WHEEL_SEPARATION;
//   return vv;
// }
...
int main() {
  ...
  while(1) {
    ...
    //   }
    //   vel_chegada.lin = (pacote_recebido[0] - '0')*100 + (pacote_recebido[1] - '0')*10 + (pacote_recebido[2] - '0');
    //   vel_chegada.ang = (pacote_recebido[3] - '0')*100 + (pacote_recebido[4] - '0')*10 + (pacote_recebido[5] - '0');
    //   cmd_vel_callback(vel_chegada);
    //   rasp.printf("v%d%d", vel_chegada.lin, vel_chegada.ang);
    // }
